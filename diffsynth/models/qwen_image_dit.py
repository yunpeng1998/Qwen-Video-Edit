# GRID-TILE PATCHED COPY -- destination in the DiffSynth-Studio checkout:
#   DiffSynth-Studio/diffsynth/models/qwen_image_dit.py
# (on the cluster: /mnt/home/repos/diffsynth-studio/diffsynth/models/qwen_image_dit.py)
#
# Diff vs upstream: QwenEmbedRope._expand_pos_freqs_if_needed and
# QwenEmbedRope.forward additionally accept dict-format img_shapes entries
# carrying an explicit frame index + spatial offsets inside a larger virtual
# canvas, so an image grid can be VAE-encoded tile-by-tile while each tile's
# tokens keep the exact RoPE positions they would have had inside the
# whole-grid encode. Legacy 3-tuple entries behave bit-identically to
# upstream. See diffsynth_grid_patch/README.md.
...
class QwenEmbedRope(nn.Module):
    ...
    def _expand_pos_freqs_if_needed(self, video_fhw, txt_seq_lens):
        # GRID-TILE PATCH: entries may be dicts (see forward() below) carrying
        # spatial offsets inside a larger virtual canvas -- for sizing purposes
        # such an entry needs positions up to its FULL canvas dims, not just
        # its own tile dims.
        if isinstance(video_fhw, list):
            frames = [i["frame"] + 1 if isinstance(i, dict) else i[0] for i in video_fhw]
            heights = [i["full_height"] if isinstance(i, dict) else i[1] for i in video_fhw]
            widths = [i["full_width"] if isinstance(i, dict) else i[2] for i in video_fhw]
            video_fhw = (max(frames), max(heights), max(widths))
    ...
    def forward(self, video_fhw, txt_seq_lens, device):
        ...
        for idx, fhw in enumerate(video_fhw):
            # GRID-TILE PATCH: an entry may be a dict describing one tile of a
            # larger virtual canvas (an image grid encoded tile-by-tile):
            #   {"frame": explicit frame-axis index (NOT the list position),
            #    "height"/"width": tile token dims,
            #    "h_off"/"w_off": tile's top-left token offset in the canvas,
            #    "full_height"/"full_width": full canvas token dims}
            # The tile's tokens get exactly the same (frame, h, w) RoPE
            # positions they would have had if the full canvas had been
            # encoded as one image (legacy 3-tuple path below) -- the only
            # difference is that tokens arrive tile-by-tile instead of in one
            # raster block, which is fine since attention is permutation
            # equivariant given per-token positions.
            if isinstance(fhw, dict):
                frame_idx = fhw["frame"]
                height, width = fhw["height"], fhw["width"]
                h_off, w_off = fhw["h_off"], fhw["w_off"]
                full_h, full_w = fhw["full_height"], fhw["full_width"]
                rope_key = f"grid_{frame_idx}_{height}_{width}_{h_off}_{w_off}_{full_h}_{full_w}"
                if rope_key not in self.rope_cache:
                    seq_lens = height * width
                    freqs_pos = self.pos_freqs.split([x // 2 for x in self.axes_dim], dim=1)
                    freqs_neg = self.neg_freqs.split([x // 2 for x in self.axes_dim], dim=1)
                    freqs_frame = freqs_pos[0][frame_idx : frame_idx + 1].view(1, 1, 1, -1).expand(1, height, width, -1)
                    if self.scale_rope:
                        axis_h = torch.cat([freqs_neg[1][-(full_h - full_h // 2):], freqs_pos[1][: full_h // 2]], dim=0)
                        axis_w = torch.cat([freqs_neg[2][-(full_w - full_w // 2):], freqs_pos[2][: full_w // 2]], dim=0)
                    else:
                        axis_h = freqs_pos[1][:full_h]
                        axis_w = freqs_pos[2][:full_w]
                    freqs_height = axis_h[h_off : h_off + height].view(1, height, 1, -1).expand(1, height, width, -1)
                    freqs_width = axis_w[w_off : w_off + width].view(1, 1, width, -1).expand(1, height, width, -1)
                    freqs = torch.cat([freqs_frame, freqs_height, freqs_width], dim=-1).reshape(seq_lens, -1)
                    self.rope_cache[rope_key] = freqs.clone().contiguous()
                vid_freqs.append(self.rope_cache[rope_key])
                if self.scale_rope:
                    max_vid_index = max(full_h // 2, full_w // 2, max_vid_index)
                else:
                    max_vid_index = max(full_h, full_w, max_vid_index)
                continue
